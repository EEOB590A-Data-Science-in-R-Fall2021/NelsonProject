...
#Your turn 1: Use filter to keep only rows with "native" webs where the web is 
# absent, and the duration is less than or equal to 4. 
# How many rows and variables does this produce?
transplant <- transplant %>%
  mutate(duration = enddate - startdate) %>%
  mutate(duration = as.numeric(duration))

(transplant_native <- transplant %>%
    filter(native == "yes" & webpres =="no" & duration >=4)) 
# * YT1.1: 9 rows x 15 variables ----

#Your turn 2: keep rows where startdate is before 2013-07-30 and end date 
# is before 2013-08-02 and webpres is yes. 
(transplant_date <- transplant %>%
   filter(startdate < "2013-07-30" & enddate < "2013-08-02" & webpres =="yes")) 
# * YT1.2: 13 rows x 15 variables ----
...
# Your turn: Calculate the mean, median, min, and max duration for webs with 
# and without netting on Guam and Saipan, and calculate the number of webs 
# in each group
transplant_web <- transplant %>%
  group_by(island, webpres) %>%
  summarize (avgDur = mean(duration),
...
# * YT4.1 ----
# A tibble: 4 x 7
# island webpres avgDur medDur minDur maxDur  nDur
# <chr>  <chr>    <dbl>  <dbl>  <dbl>  <dbl> <int>
# 1 guam   no        4.77      5      1      7    31
# 2 guam   yes       4.45      4      3      7    11
# 3 saipan no        4.75      5      1      9    12
# 4 saipan yes       2.14      1      1      8    37


# Your turn, advanced version: 
# 1. Calculate mean duration of webs with and without netting at each site on 
# each island. 
# 2. Add these average duration values to the tidy transplant dataset (the one 
# we loaded at the start of this script). 
# 3. Select just the island, site, netting, mean_dur, web, websize, duration 
# columns. 
# 4. Add a column that calculates difference between duration of each web and 
# the mean_duration for all webs at that island/site/netting combo. 
transplant_net <- transplant %>%
  group_by(island, site, netting) %>%
  summarize (avgDur = mean(duration),
             .groups = "drop") %>%
  right_join(transplant, by = c("island", "site", "netting")) %>%
  select(island, site, netting, avgDur, web, websize, duration) %>%
  mutate(diffDur = duration - avgDur)

# * YT4.2 ----
# A tibble: 91 x 8
# island site  netting avgDur web   websize duration diffDur
#<chr>  <chr> <chr>    <dbl> <chr>   <dbl>    <dbl>   <dbl>
# 1 guam   anao  no        4.71 I'V        70        5   0.286
# 2 guam   anao  no        4.71 I'T        63        3  -1.71 
# 3 guam   anao  no        4.71 I'S        57        7   2.29 
# 4 guam   anao  no        4.71 J'C        38        3  -1.71 
# 5 guam   anao  no        4.71 I'R        34        3  -1.71 
# 6 guam   anao  no        4.71 I'U        32        5   0.286
# 7 guam   anao  no        4.71 I'Q        25        7   2.29 
# 8 guam   anao  yes       4.17 J'A        51        3  -1.17 
# 9 guam   anao  yes       4.17 I'X        49        3  -1.17 
# 10 guam   anao  yes       4.17 I'W        48        5   0.833
# ... with 81 more rows
