...
# each island. 
# 2. Add these average duration values to the tidy transplant dataset 
# (the one we loaded at the start of this script). 
# 3. Select just the island, # site, netting, mean_dur, web, websize, duration 
# columns. 
# 4. Add a column that calculates the difference between duration of each web 
# and the mean_duration for all webs at that island/site/netting combo. 
...
## Your Turn ##
(trans_netting <- transplant %>%
    group_by(island, site, netting) %>%
    summarize(meanDur = mean(duration), .groups = "drop") %>%
    right_join(transplant, by = c("island", "site", "netting")) %>%
    select(island, site, netting, meanDur, web, websize, duration) %>%
    mutate(web_durDiff = duration - meanDur))

(size_durDiffpos <- trans_netting %>%
    filter(web_durDiff > 0) %>%
    summarise(avg_pos = mean(websize), 
              med_pos = median(websize), 
              sd_pos = sd(websize), 
              max_pos = max(websize), 
              min_pos = min(websize, na.rm = TRUE),
              count_pos = n()))

(size_durDiffneg <- trans_netting %>%
    filter(web_durDiff < 0) %>%
    summarise(avg_neg = mean(websize), 
              med_neg = median(websize), 
              sd_neg = sd(websize), 
              max_neg = max(websize), 
              min_neg = min(websize, na.rm = TRUE),
              count_neg = n()))
